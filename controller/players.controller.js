import { Create, Update, Delete, getOne, getAll} from "../helpers/tournament.service.js"
const table = "players"
const TableId = "player_id"

export const playerController ={
    create: async (req, res, next)=>{
        try{
            const data = req.body
            const { full_name, date_of_birth, position, team_id, jersey_number} = data
            if(!full_name || !date_of_birth || !position || !team_id ||!jersey_number) return res.status(400).json({message: "Player full name, Date of birth, Position, Team id and Jersey Number are required"})

            const result = await Create(data, table)
            res.status(201).json(result)
        }catch(err){
            next(err)
        }
    },
    update: async (req, res, next)=>{
        try{
            const { id } = req.params
            const data = req.body

            const result = await Update(id, data, table, TableId)
            if (!result) return res.status(404).json({ message: "Player Not Found"})

            res.status(200).json(result)
        }catch(err){
            next(err)
        }
    },
    delete: async(req, res, next)=>{
        try{
            const { id } = req.params

            const result = await Delete(id, table, TableId)
            if (!result) return res.status(404).json({ message: "Player Not Found"})
            
            res.status(200).json({message: "Player Deleted Successfully", Player: result})
        }catch(err){
            next(err)
        }
    },
    GetOne: async (req, res, next)=>{
        try{
            const { id } = req.params

            const result = await getOne(id, table)
            if (!result) return res.status(404).json({ message: "Player Not Found"})
            
            res.status(200).json(result)
        }catch(err){
            next(err)
        }
    },
    GetAll: async(req, res, next)=>{
        try{
            const result = await getAll(table)
            if (!result) return res.status(404).json({ message: "Player Not Found"})
            
            res.status(200).json(result)
        }catch(err){
            next(err)
        }
    }
}